...
class Revolution:
    ...
    def train_one_batch(self, obs):
        ...
        memory = torch.cat([torch.zeros(self.batch_size, self.acmodel.memory_dim), torch.zeros(self.batch_size, self.acmodel.memory_dim)],
    ...
    def load_generated_demos(self, path, max_steps=128):
        ...
        for demo in demos:
            ...
            p.instructions = torch.from_numpy(np.array([vocab[token] for token in tokens])).repeat(len(demo),1).to(self.device)
            # dummy elements
    ...
    def prepare_batch(self,step):
        episodes = self.load_generated_demos("../scripts/demos/train/1000Episodes.pkl")
