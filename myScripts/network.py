...
class Net(torch.nn.Module):
    def __init__(self, instr_embed_dim,ppo_embed_dim, action_dim, n_lstm, image_dim=128,device="gpu",own_net=True):
        ...
        self.myLstm1=torch.nn.LSTM(self.input_length,n_lstm)
    ...
    def forward_own_net(self,input,hidden ):
    ...
    def forward(self, input,hidden):
        ...
        if self.own_net:
            pred,hidden = self.forward_own_net(input,hidden)
        else:
            pred = self.forward_no_own_net(input,hidden)
        # with torch.no_grad():
